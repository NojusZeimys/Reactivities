...
export default function ActivityDetailsHeader({ activity }: Props) {
  ...
  return (
    <Card sx={{ position: "relative", mb: 2, backgroundColor: "transparent", overflow: "hidden" }}>
      {isCancelled && <Badge sx={{ position: "absolute", left: 40, top: 20, zIndex: 1000 }} color="error" badgeContent="Cancelled" />}
      <CardMedia component="img" height="300" image={`/images/categoryImages/${activity.category}.jpg`} alt={`${activity.category} image`} />
      <Box
        sx={{
          position: "absolute",
          bottom: 0,
        ...
        <Box sx={{ display: "flex", gap: 2 }}>
          {isHost ? (
            ...
            <Button variant="contained" color={isGoing ? "primary" : "info"} onClick={() => {}} disabled={isCancelled || loading}>
